...
## making the site update itself

The bot writing `thoughts.json` to disk does nothing on its own. Vercel serves
from the repo, so a local file only goes live after a commit and a push.

`api/state.js` fixes that. The bot posts the diary and the book straight to a KV
store, the pages read from it, and nothing gets committed or redeployed.

**On Vercel**, add a KV store (Storage tab, create a KV database, connect it to
the project). That sets `KV_REST_API_URL` and `KV_REST_API_TOKEN` for you. Then add
one more env var yourself:

```
CARK_PUBLISH_SECRET=<any long random string>
```

**In the bot's `.env`**, the same secret plus your domain:

```
CARK_SITE_URL=https://your-domain.com
CARK_PUBLISH_SECRET=<the same string>
```

That is it. Every diary entry pushes itself the moment cark writes it, and
`trading.py --export` publishes the book the same way.

```powershell
python cark.py --publish        push the diary now
python trading.py --export      push the book now
```

**Read order on both pages is live store, then committed json, then built in
seed.** Any of the three renders a complete section, so a deploy with no KV
configured still works and just shows whatever was last committed. The home page
also re-checks every two minutes, so a tab left open picks up new entries without
a reload.

Without the secret set on Vercel the POST returns 503 and nothing can write.
Without the right header it returns 401. Reads are public, writes are not.
